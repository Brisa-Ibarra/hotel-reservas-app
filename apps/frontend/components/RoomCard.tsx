import { Bed, Star, Wrench, DoorClosed } from 'lucide-react';
...
const statusBadge: Record<string, { label: string; className: string; icon?: React.ReactNode }> = {
    available: { label: 'Disponible', className: 'bg-[#C4A35A] text-[#2D4A2D]' },
    under_maintenance: { label: 'Mantenimiento', className: 'bg-red-500/90 text-white', icon: <Wrench size={12} /> },
    occupied: { label: 'Ocupada', className: 'bg-blue-500/90 text-white', icon: <DoorClosed size={12} /> },
};
    ...
    return (
        <div className="group border border-[#C4A35A]/20 rounded-2xl bg-white shadow-sm hover:shadow-xl hover:shadow-[#2D4A2D]/10 hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden">
            <div className="relative bg-gradient-to-r from-[#2D4A2D] to-[#3D5A3D] px-5 py-4 flex justify-between items-center overflow-hidden">
                ...
                <span className={`relative text-xs px-3 py-1 rounded-full font-semibold ${badge.className}`}>
                    {badge.icon ? (
